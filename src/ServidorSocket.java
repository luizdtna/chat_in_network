...
public class ServidorSocket extends Thread {
    ...
    public static void main(String args[]) {
        //HashMap= armazena os computadores para enviar as mensagens
    ...
    public void run() {
        try {
            ...
            if (armazena(this.nomeCliente)) {
                ...
                saida.println(this.nomeCliente + " : Conectado ao Servidor!");
                
                //Testendo esse comando abaixo
                String[] entrouChat = {"no Chat"};
                send(saida, "entrou ", entrouChat);
            ...
            while (msg != null && !(msg[0].trim().equals("")) && !msg[0].contains("12qw78asç")) {
                send(saida, " escreveu >> ", msg);
                msg = entrada.readLine().split(":");
            }
            System.out.println(this.nomeCliente + " saiu do bate-papo!");
            String[] out = {" do bate-papo!"}; //Ta assim pq recebe como parametro um array
            send(saida, " saiu", out);
    ...
    public void send(PrintStream saida, String acao, String[] msg) {
        ...
        for (Map.Entry<String, PrintStream> cliente : MAP_CLIENTES.entrySet()) {
            ...
            if (chat != saida) {
                ...
                } else if (msg[1].equalsIgnoreCase(cliente.getKey())) {
                    chat.println(this.nomeCliente + acao + msg[0]);
                    break out;
                }
            }
